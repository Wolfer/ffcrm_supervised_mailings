class MailingsController < ApplicationController
  ...
  #FIXME after_filter  :update_recently_viewed, :only => :show
  ...
  #----------------------------------------------------------------------------
  def show
    @mailing = Mailing.my.find(params[:id])
  ...
  def attachment
    ...
    if @mailing.attc?
      send_file "#{Rails.root}/files/supervised_mailings/attachments/#{@mailing.id}/#{@mailing.attc_file_name}", :type => @mailing.attc_content_type, :filename=> @mailing.attc_file_name, :disposition => 'attachment'      
    else
  ...
  #----------------------------------------------------------------------------
  def options_mails
    @mailing = Mailing.my.find(params[:related]) if params[:related]
  ...
  #----------------------------------------------------------------------------
  def redraw_mails
    @mailing = Mailing.my.find(params[:related])
    @users = User.except(@current_user).all      
  ...
  def get_mailings(options = { :page => nil, :query => nil })
    ...
    if current_query.blank?
      if current_filter.empty? || current_filter == "all"  
        ...
        Mailing.my(records).find_by_status(current_filter)
      end
    else
      if current_filter.empty? || current_filter == "all"  
        Mailing.my(records).search(current_query)
      else  
        Mailing.my(records).search(current_query).find_by_status(current_filter)
      end
    end.paginate(pages)
